...
int main(int argc, char * argv[])
{
  ...
  /* main loop: get and send lines of text */
	...
	for (size_t i = 0; i < 300000; i++) {
		l = sendto(s, buf, strlen(buf), 0, (struct sockaddr *)&sin, remoteaddresslength);
		if(l < 0){
			printf("ERRO NO SEND");
		}
	}
  ...
  //   if(!strcmp(inet_ntoa(remoteaddress.sin_addr), inet_ntoa(sin.sin_addr)) &&
  //       remoteaddress.sin_port == sin.sin_port){
  //         shouldReadFromStdin = 1;
  //     if (recvlen >= 0) {
  //         buf[recvlen] = 0;
  //         printf("%s", buf);
  //     }
  //   }else{
  //     fprintf(stdout, "discarding message\n");
  //     shouldReadFromStdin = 0;
  //   }
  // }
  close(s);
